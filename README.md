...
바탕화면 **「게임뉴스 실행」** 더블클릭 → 서버가 백그라운드(창 없이)로 켜지고 잠시 뒤 브라우저가 자동으로 열립니다. 끄려면 **「게임뉴스 종료」** 더블클릭.

- `start.vbs` : 백엔드+프론트엔드를 숨김 실행 후 http://localhost:3000 열기
- `stop.vbs`  : 8000·3000 포트의 서버 종료
- (최초 1회만) 아래 "빠른 시작"으로 의존성 설치가 끝나 있어야 함
